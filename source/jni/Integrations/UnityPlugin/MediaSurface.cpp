...
{
	vInfo( "BindMediaSurface" );
	if ( SurfaceObject == NULL )
	{
		vInfo( "SurfaceObject == NULL" );
		abort();
	}
	TexId = toTexId;
	TexIdWidth = -1;
...
void MediaSurface::Update()
{
	...
	{
		vInfo( "Allocating GL objects" );

		UnitSquare.createPlaneQuadGrid( 1, 1 );

		CopyMovieProgram.initShader("uniform highp mat4 Mvpm;\n"
	...
	{
		vInfo( "New surface size:"<<width<<"x"<<height);
