...
#include "api/SystemActivities.h"
...
public:
						...
						ErrorMessageEndTime( -1.0 )
					{
						...
						{
							sensorForView[i].Predicted.Pose.Orientation = NervGear::Quat<float>();
						}
					}
...
OCULUS_EXPORT void OVR_Platform_StartUI( const char * commandString )
{
	...
	int imageSize = 0;
	if ( !ovr_FindEmbeddedImage( up.OvrMobile, imageName.toCString(), imageBuffer, imageSize ) )
	{
...
OCULUS_EXPORT float OVR_GetBatteryLevel()
{
	...
	return NervGear::Alg::Clamp( static_cast<float>( state.level ) / 100.0f, 0.0f, 1.0f );
}
