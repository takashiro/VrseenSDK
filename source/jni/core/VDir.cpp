...
bool VDir::contains(VPath path, mode_t mode) {
    ...
    void VDir::makePath(const VPath &path, mode_t mode) {
        ...
        OVR_sprintf(vpath, sizeof(vpath), "%s", path.toCString());
    ...
    VArray<VString> VDir::Search(const VArray<VString> &searchPaths,
            const VString &relativePath) {
