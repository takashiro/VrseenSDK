...
#include "Capture_AsyncStream.h"
#include "Capture_StandardSensors.h"
...
namespace Capture
{
    ...
    ***********************************/
    ...
    static void SendLabelPacket(const Label &label)
    {
        ...
        {
            LabelPacket packet;
            packet.labelID = label.identifier();
    ...
    ***********************************/
    ...
    class Server : public Thread
    {
        ...
        private:
            ...
            {
                SetThreadName("NervGear::Capture");
                while(m_listenSocket && !QuitSignaled())
                {
                    ...
                    for(Label *l=Label::head(); l; l=l->next())
                    {
                        SendLabelPacket(*l);
                    }
    ...
    ***********************************/
    ...
    Label *Label::head(void)
    {
        Label *head = s_head;
        return head;
    }

    Label *Label::next(void) const
    {
        return m_next;
    }
