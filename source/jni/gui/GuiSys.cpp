...
int OvrGuiSysLocal::FindMenuIndex(const VString &menuName) const
{
    for ( int i = 0; i < Menus.length(); ++i )
    {
        if (menuName.icompare(Menus[i]->name()) == 0) {
            return i;
        }
    }
    return -1;
...
//==============================
// OvrGuiSysLocal::FindActiveMenuIndex
int OvrGuiSysLocal::FindActiveMenuIndex(const VString &menuName) const
{
    for (int i = 0; i < ActiveMenus.length(); ++i) {
        if (menuName.icompare(ActiveMenus[i]->name()) == 0) {
            return i;
        }
    }
    return -1;
...
void OvrGuiSysLocal::openMenu(App * app, OvrGazeCursor & gazeCursor, const VString &menuName)
{
    int menuIndex = FindMenuIndex( menuName );
    if ( menuIndex < 0 )
    {
