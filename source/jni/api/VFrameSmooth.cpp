...
void Java_com_vrseen_VrLib_nativeVsync(JNIEnv *, jclass, jlong frameTimeNanos) {
    ...
    state.vsyncPeriodNano = 1e9 / 59.46;
...
struct warpSource_t {
    ...
    VTimeWarpParms WarpParms;                    // passed into WarpSwap()

    warpSource_t()
            : MinimumVsync(0), FirstDisplayedVsync{0, 0}, disableChromaticCorrection(false),
              GpuSync(nullptr) {
    }
};
...
swapProgram_t spAsyncFrontBufferPortrait = {
        false, false, {0.5, 1.0}, {{1.0, 1.5}, {1.5, 2.0}}// //zx_note vivo反向刷屏修改
};
...
void EyeRectLandscape(const VDevice *device, int eye, int &x, int &y, int &width, int &height) {
    ...
    x = device->widthbyPixels / 4 - pixelRadius +
        ((eye == 0) ? -horizontalShiftPixels : device->widthbyPixels / 2 + horizontalShiftPixels);
    y = 0;
    width = pixelDiameter;
...
static bool IsContextPriorityExtensionPresent() {
    ...
    if (!eglQueryContext(eglGetCurrentDisplay(), eglGetCurrentContext(),
                         EGL_CONTEXT_PRIORITY_LEVEL_IMG, &currentPriorityLevel)
...
#define THREAD_STATUS_SUSPEND       0x00000001
#define SURFACE_RESOLUTION_CHANGED  0x00000002
#define SURFACE_REVOLUTION_SETUPED  0x00000004

    Private(bool wantSingleBuffer, VFrameSmooth::HardwareRefreshDirection direction) :
        ...
        if (!eglQueryContext(m_eglStatus.m_display, m_eglShareContext, EGL_CONTEXT_CLIENT_VERSION,
                             (EGLint *) &m_eglClientVersion)) {
            vFatal("eglQueryContext EGL_CONTEXT_CLIENT_VERSION failed");
        }
        //VInfo( "Current EGL_CONTEXT_CLIENT_VERSION:%i", m_eglClientVersion );

        // It is wasteful for the main config to be nothing but a color buffer.
        ...
        const int createErr = pthread_create(&m_warpThread, NULL, &ThreadStarter, this);
        if (createErr != 0) {
    ...
    void destroy() {
        ...
        if (m_warpThread != 0) {
            ...
            eglMakeCurrent(EGL_NO_DISPLAY, EGL_NO_SURFACE, EGL_NO_SURFACE, m_eglStatus.m_context);
            // Destroy the pbuffer surface that was attached to the calling context.
            if (EGL_FALSE ==
                eglDestroySurface(m_eglStatus.m_display, m_eglStatus.m_pbufferSurface)) {
    ...
    void warpSwapInternal(const VTimeWarpParms &parms);
    ...
    bool isSumsungDevice();

    const VGlShader &programForParms(const VTimeWarpParms &parms,
                                     const bool disableChromaticCorrection) const;
    ...
    VMutex m_mutex;
    int32_t m_flags;                    // flags defined above
    VWaitCondition m_cond_suspended;
    VWaitCondition m_cond_suspending;
    VWaitCondition m_cond_surfaceChanged;
    VFrameSmooth::HardwareRefreshDirection m_direction;
};
...
double    VFrameSmooth::Private::getFractionalVsync() {
    ...
    const double vsync =
            (double) state.vsyncCount + (double) (t - state.vsyncBaseNano) / state.vsyncPeriodNano;
    return vsync;
}
...
double    VFrameSmooth::Private::framePointTimeInSeconds(const double framePoint) const {
    ...
    const double seconds =
            (state.vsyncBaseNano + (framePoint - state.vsyncCount) * state.vsyncPeriodNano) * 1e-9;
    return seconds;
}

float    VFrameSmooth::Private::sleepUntilTimePoint(const double targetSeconds,
                                                    const bool busyWait) {
...
void VFrameSmooth::Private::threadFunction() {
    ...
    int syscallres, mask = 1;
    pid_t pid = gettid();
    syscallres = syscall(__NR_sched_setaffinity, pid, sizeof(mask), &mask);
    if (syscallres) {
        int num = sysconf(_SC_NPROCESSORS_CONF);
        vFatal("Failed to set thread affinity with " << num << " cores!");
    }

    jint rtn = VrLibJavaVM->AttachCurrentThread(&m_jni, 0);
    if (rtn != JNI_OK) {
    ...
    for (double vsync = 0; ; vsync++) {
        {
            m_mutex.lock();
            if (m_flags & THREAD_STATUS_SUSPEND) {
                m_cond_suspended.notify();
                m_cond_suspending.wait(&m_mutex);
            }

            if (m_eglMainThreadSurface == EGL_NO_SURFACE) {
                m_cond_surfaceChanged.wait(&m_mutex);
            }

            if (m_flags & SURFACE_RESOLUTION_CHANGED) {

                m_flags &= ~SURFACE_RESOLUTION_CHANGED;
                // if surface existed, release it first
                if (m_flags & SURFACE_REVOLUTION_SETUPED) {
                    destroyFrameworkGraphics();
                    vInfo("Destroy!");
                } else {
                    m_flags |= SURFACE_REVOLUTION_SETUPED;
                }

                // TODO: release current surface before creating new

                eglMakeCurrent(m_eglStatus.m_display, m_eglMainThreadSurface,
                               m_eglMainThreadSurface, m_eglStatus.m_context);
                m_screen.initForCurrentSurface(m_jni, m_wantSingleBuffer, m_buildVersionSDK);
                m_screen.getScreenResolution(m_device->widthbyPixels, m_device->heightbyPixels);
        ...
        if (abs(current - vsync) > 2.0) {
            //VInfo( "Changing vsync from %f to %f", vsync, current );
            vsync = current;
        }
    ...
    rtn = VrLibJavaVM->DetachCurrentThread();
    if (rtn != JNI_OK) {
        vFatal("DetachCurrentThread returned" << rtn);
    }
...
* Startup()
*/
VFrameSmooth::VFrameSmooth(bool wantSingleBuffer, HardwareRefreshDirection direction)
        : d(new Private(wantSingleBuffer, direction)) {
...
VFrameSmooth::~VFrameSmooth() {
    d->m_mutex.lock();
    d->m_shutdownRequest.setState(true);
    if (d->m_flags & THREAD_STATUS_SUSPEND) {
        d->m_flags &= ~THREAD_STATUS_SUSPEND;
        d->m_cond_suspending.notify();
    }
    d->m_mutex.unlock();
    d->destroy();
    delete d;
...
int VFrameSmooth::threadId() const {
    return d->m_warpThread;
}

void VFrameSmooth::pause() {
    d->m_mutex.lock();
    if ((d->m_flags & THREAD_STATUS_SUSPEND) == 0) {
        d->m_flags |= THREAD_STATUS_SUSPEND;
        d->m_cond_suspended.wait(&d->m_mutex);
    }
    d->m_mutex.unlock();

}

void VFrameSmooth::setupSurface(EGLSurface surface) {
    d->m_mutex.lock();
    d->m_eglMainThreadSurface = surface;
    d->m_flags |= SURFACE_RESOLUTION_CHANGED;
    d->m_cond_surfaceChanged.notify();
    d->m_mutex.unlock();
}

void VFrameSmooth::resume() {
    d->m_mutex.lock();
    if (d->m_flags & THREAD_STATUS_SUSPEND) {
        d->m_flags &= ~THREAD_STATUS_SUSPEND;
        d->m_cond_suspending.notify();
...
void VFrameSmooth::Private::warpThreadInit() {
    ...
    //VGlOperation glOperation;
    m_eglStatus.m_context = eglCreateContext(m_eglStatus.m_display, m_eglStatus.m_config,
                                             m_eglShareContext, contextAttribs);
    ...
    for (int i = 0; i < MAX_WARP_SOURCES; i++) {
        ...
        if (ws.GpuSync) {
            if (EGL_FALSE == m_eglStatus.eglDestroySyncKHR(m_eglStatus.m_display, ws.GpuSync)) {
                vError("eglDestroySyncKHR returned EGL_FALSE");
            }
            ws.GpuSync = 0;
        }
    }
...
const VGlShader &VFrameSmooth::Private::programForParms(const VTimeWarpParms &parms,
                                                        const bool disableChromaticCorrection) const {
    int program = VAlgorithm::Clamp((int) parms.WarpProgram, (int) WP_SIMPLE,
                                    (int) WP_PROGRAM_MAX - 1);
...
void VFrameSmooth::Private::bindWarpProgram(const warpSource_t &currentWarpSource,
                                            const VMatrix4f timeWarps[2][2],
                                            const VMatrix4f rollingWarp,
                                            const int eye,
                                            const double vsyncBase /* for spinner */ ) const {
    ...
    const VGlShader &warpProg = programForParms(currentWarpSource.WarpParms,
                                                currentWarpSource.disableChromaticCorrection);
    glUseProgram(warpProg.program);

    // Set the shader parameters.
    glUniform1f(warpProg.uniformColor, currentWarpSource.WarpParms.ProgramParms[0]);

    glUniformMatrix4fv(warpProg.uniformModelViewProMatrix, 1, GL_FALSE,
                       landscapeOrientationMatrix.transposed().cell[0]);
    glUniformMatrix4fv(warpProg.uniformTexMatrix, 1, GL_FALSE,
                       timeWarps[0][0].transposed().cell[0]);
    glUniformMatrix4fv(warpProg.uniformTexMatrix2, 1, GL_FALSE,
                       timeWarps[0][1].transposed().cell[0]);
    if (warpProg.uniformTexMatrix3 > 0) {
        glUniformMatrix4fv(warpProg.uniformTexMatrix3, 1, GL_FALSE,
                           timeWarps[1][0].transposed().cell[0]);
        glUniformMatrix4fv(warpProg.uniformTexMatrix4, 1, GL_FALSE,
                           timeWarps[1][1].transposed().cell[0]);
    }
    if (warpProg.uniformTexMatrix5 > 0) {
        glUniformMatrix4fv(warpProg.uniformTexMatrix5, 1, GL_FALSE,
                           rollingWarp.transposed().cell[0]);
    }
    ...
    if (warpProg.uniformRotateScale > 0) {
        const float angle = framePointTimeInSeconds(vsyncBase) * M_PI *
                            currentWarpSource.WarpParms.ProgramParms[0];
        const VVect4f RotateScale(sinf(angle), cosf(angle),
                                  currentWarpSource.WarpParms.ProgramParms[1], 1.0f);
...
static void BindEyeTextures(const warpSource_t &currentWarpSource, const int eye) {
    ...
    if (currentWarpSource.WarpParms.WarpProgram == WP_MASKED_PLANE_EXTERNAL
        ...
        if (currentWarpSource.WarpParms.WarpOptions & SWAP_OPTION_INHIBIT_SRGB_FRAMEBUFFER) {
            glTexParameteri(GL_TEXTURE_EXTERNAL_OES, VEglDriver::GL_TEXTURE_SRGB_DECODE_EXT,
                            VEglDriver::GL_SKIP_DECODE_EXT);
        }
        else {
            glTexParameteri(GL_TEXTURE_EXTERNAL_OES, VEglDriver::GL_TEXTURE_SRGB_DECODE_EXT,
    ...
    if (currentWarpSource.WarpParms.WarpProgram == WP_MASKED_CUBE ||
        currentWarpSource.WarpParms.WarpProgram == WP_CHROMATIC_MASKED_CUBE) {
        glActiveTexture(GL_TEXTURE1);
    ...
    if (currentWarpSource.WarpParms.WarpProgram == WP_CUBE ||
        currentWarpSource.WarpParms.WarpProgram == WP_CHROMATIC_CUBE) {
        for (int i = 0; i < 3; i++) {
            glActiveTexture(GL_TEXTURE1 + i);
            glBindTexture(GL_TEXTURE_CUBE_MAP,
                          currentWarpSource.WarpParms.Images[eye][1].PlanarTexId[i]);
            if (currentWarpSource.WarpParms.WarpOptions & SWAP_OPTION_INHIBIT_SRGB_FRAMEBUFFER) {
                glTexParameteri(GL_TEXTURE_CUBE_MAP, VEglDriver::GL_TEXTURE_SRGB_DECODE_EXT,
                                VEglDriver::GL_SKIP_DECODE_EXT);
            }
    ...
    if (currentWarpSource.WarpParms.WarpProgram == WP_LOADING_ICON ||
        currentWarpSource.WarpParms.WarpProgram == WP_CHROMATIC_LOADING_ICON) {
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, currentWarpSource.WarpParms.Images[eye][1].TexId);
        if (currentWarpSource.WarpParms.WarpOptions & SWAP_OPTION_INHIBIT_SRGB_FRAMEBUFFER) {
            glTexParameteri(GL_TEXTURE_2D, VEglDriver::GL_TEXTURE_SRGB_DECODE_EXT,
...
void VFrameSmooth::Private::warpToScreen(const double vsyncBase_, const swapProgram_t &swap) {
    ...
    const warpSource_t &latestWarpSource = m_warpSources[m_eyeBufferCount.state() %
                                                         MAX_WARP_SOURCES];
    // switch to sliced rendering
    if (latestWarpSource.WarpParms.WarpOptions & SWAP_OPTION_USE_SLICED_WARP)// //zx_note 未执行
    {
    ...
    for (int eye = 0; eye <= 1; ++eye) {
        ...
        // vInfo( "Vsync %f:%i sleep %f", vsyncBase, eye, secondsToSleep );

        const double sleepTargetVsync = vsyncBase + swap.deltaVsync[eye];
        double sleepTargetTime = framePointTimeInSeconds(sleepTargetVsync);

        sleepUntilTimePoint(sleepTargetTime, /*false*/true);
        ...
        if (eye == 0) {
            ...
            for (back = 0; back < MAX_WARP_SOURCES - 1; back++) {
                ...
                if (testWarpSource.GpuSync == 0) {
                    //VInfo( "thisEyeBufferNum %lli had 0 sync", thisEyeBufferNum );
                    break;
                }
                ...
                const EGLint wait = m_eglStatus.eglClientWaitSyncKHR(m_eglStatus.m_display,
                                                                     testWarpSource.GpuSync,
        ...
        VMatrix4f velocity;
        const int velocitySteps = std::min(3, (int) ((long long) vsyncBase -
                                                     currentWarpSource.MinimumVsync));
        ...
        for (int scan = 0; scan < 2; scan++) {
            ...
            timeWarps[0][scan] =
                    VMatrix4f(currentWarpSource.WarpParms.Images[eye][0].TexCoordsFromTanAngles) *
                    warp;
            if (dualLayer) {
                if (currentWarpSource.WarpParms.WarpOptions &
                    SWAP_OPTION_FIXED_OVERLAY) {    // locked-to-face HUD
                    timeWarps[1][scan] = VMatrix4f(
                            currentWarpSource.WarpParms.Images[eye][1].TexCoordsFromTanAngles);
                }
                else {    // locked-to-world surface
...
//        VsyncState state = UpdatedVsyncState.state();
//        double cur_time = VTimer::TicksNanos();
//        static double sleepTargetTime = 0;
//        if(eye == 0) {
//            double vsync_count = (cur_time - state.vsyncBaseNano) / state.vsyncPeriodNano;
//
//            if(m_direction == VFrameSmooth::HWRD_BT) {
//                if(vsync_count < 0.5) {
//                    sleepTargetTime = (state.vsyncBaseNano + floor(vsync_count) * state.vsyncPeriodNano) * 1e-9;
//                    continue;
//                } else {
//                    sleepTargetTime = (state.vsyncBaseNano + ceil(vsync_count) * state.vsyncPeriodNano) * 1e-9;
//                }
//            } else {
//                sleepTargetTime = (state.vsyncBaseNano + (floor(vsync_count) + 0.5) * state.vsyncPeriodNano) * 1e-9;
//            }
//        } else {
//            sleepTargetTime += state.vsyncPeriodNano * 1 / 2 * 1e-9;
//        }
//
//        if(sleepTargetTime * 1e9 < cur_time) {
//            static int dropped = 0;
//            dropped++;
//            vError("Current time is " << (cur_time - state.vsyncBaseNano) / 1000 << " ms!");
//            vError("VSync calculated is " << (sleepTargetTime * 1e6 - state.vsyncBaseNano / 1000) << " ms!");
//            vError("Total " << frame_count << " frames are drawed, "<< dropped << "frames are dropped! eye = " << eye);
//            continue;
//        }
//
//        frame_count++;
//        sleepUntilTimePoint( sleepTargetTime, true );
        ...
        double justBeforeFinish = VTimer::Seconds();

        if (m_screen.isFrontBuffer()) {
            // vInfo("m_screen.isFrontBuffer true");
        ...
        if (latency > 0.008f) {
            //vError( "Frame " << (int)vsyncBase << "for eye " << eye << " total takes " << (latency*1000) << "ms!");
        }
    }    // for eye
    ...
    if (!m_screen.isFrontBuffer()) {
        ...
        if (VDeviceManager::getSupportedVRDeviceType() == VDeviceManager::VDevice_Type_Sumsung)
            m_screen.swapBuffers();
    }
}
...
//                }
        ...
        if (0) {    // solid color flashing test to see if slices are rendering in the right place
        ...
        if (m_screen.isFrontBuffer() &&
            ...
            // vError("%%%%%%%%%%%%%   warpToScreen isFrontBuffer");
        }
        ...
        const float latency = postFinish - justBeforeFinish;
        if (latency > 0.008f) {
            vError("Frame %i Eye %i latency %5.3f" << (int) vsyncBase << eye << latency);
        }
    }    // for screenSlice
...
void VFrameSmooth::Private::warpSwapInternal(const VTimeWarpParms &parms) {
    // Keep track of the last time WarpSwap() was called.
    m_lastWarpSwapTimeInSeconds.setState(VTimer::Seconds());
    ...
    ws.MinimumVsync = m_lastSwapVsyncCount + 2 *
                                             minimumVsyncs;    // don't use it if from same frame to avoid problems with very fast frames
    ...
    ws.disableChromaticCorrection = (
            (m_eglStatus.eglGetGpuType() & VEglDriver::GPU_TYPE_MALI_T760_EXYNOS_5433) != 0);
    ws.WarpParms = parms;
    ...
    if (EGL_FALSE == m_eglStatus.eglClientWaitSyncKHR(m_eglStatus.m_display, ws.GpuSync,
        ...
        vError("eglClientWaitSyncKHR returned EGL_FALSE");
    }
    ...
    for (; ;) {
        ...
        if (state.EyeBufferCount >= lastBufferCount) {
            ...
            if (suspendNanoSeconds < 1000 * 1000) {
                ...
                //VInfo( "WarpSwap: usleep( %lld )", suspendMicroSeconds );
...
*
*/
void VFrameSmooth::doSmooth(const VTimeWarpParms &parms) {
...
//    glBindTexture( GL_TEXTURE_2D, 0 );
    ...
    m_sliceMesh = VLensDistortion::createDistortionGrid(m_device, NUM_SLICES_PER_EYE,
                                                        calibrateFovScale, false);
    ...
    m_untexturedMvpProgram.initShader(VGlShader::getUntextureMvpVertexShaderSource(),
                                      VGlShader::getUntexturedFragmentShaderSource());
    m_debugLineProgram.initShader(VGlShader::getVertexColorVertexShaderSource(),
                                  VGlShader::getUntexturedFragmentShaderSource());
    buildWarpProgs();
}
...
//    }
    ...
    if (swapOptions & SWAP_OPTION_DRAW_CALIBRATION_LINES) {
        ...
        glUniformMatrix4fv(m_untexturedMvpProgram.uniformModelViewProMatrix, 1,
...
void VFrameSmooth::Private::buildWarpProgMatchedPair(VrKernelProgram simpleIndex,
                                                     ...
                                                     const char *simpleFragment
) {
    m_warpPrograms[simpleIndex] = VGlShader(simpleVertex, simpleFragment);
    m_warpPrograms[simpleIndex + (WP_CHROMATIC - WP_SIMPLE)] = VGlShader(simpleVertex,
                                                                         simpleFragment);
}
    ...
    buildWarpProgPair(WP_SIMPLE,
            ...
            // high quality
                      ...
                      "precision mediump float;\n"
                              ...
                              "{\n"
                              "#ifdef VIEW_ID\n"
                              "  lowp vec4 color1r = texture2D(Texture0, vec3(oTexCoord1r,VIEW_ID));\n"
                              "	 lowp vec4 color1g = texture2D(Texture0, vec3(oTexCoord1g,VIEW_ID));\n"
                              "	 lowp vec4 color1b = texture2D(Texture0, vec3(oTexCoord1b,VIEW_ID));\n"
                              "#else\n"
                              "	 lowp vec4 color1r = texture2D(Texture0, oTexCoord1r);\n"
                              "	 lowp vec4 color1g = texture2D(Texture0, oTexCoord1r);\n"
                              "	 lowp vec4 color1b = texture2D(Texture0, oTexCoord1r);\n"
                              "#endif\n"
