	...
	for ( double vsync = 0; ; vsync++ )
	{
		...
		if ( m_setSchedFifoMethodId != 0 )
		{
			...
			if ( removedSchedFifo )
			{
				...
				{
					LOG( "TimeWarp Watchdog: restored SCHED_FIFO on tid %d", m_sStartupTid );
					...
					{
						LOG( "TimeWarp Watchdog: restored SCHED_FIFO on tid %d", m_initParms.gameThreadTid );
			...
			else
			{
				...
				{
					LOG( "TimeWarp Watchdog: removed SCHED_FIFO from tid %d", m_sStartupTid );
					...
					if ( m_initParms.gameThreadTid )
					{
						LOG( "TimeWarp Watchdog: removed SCHED_FIFO from tid %d", m_initParms.gameThreadTid );
...
void TimeWarpLocal::warpThreadInit()
{
	...
	pthread_setname_np( pthread_self(), "NervGear::TimeWarp" );
