...
void HIDDeviceManager::Shutdown()
{
    ...
    LogText("NervGear::Linux::HIDDeviceManager - shutting down.\n");
}
...
bool HIDDevice::HIDInitialize(const String& path)
{
    ...
    {
        LogText("NervGear::Linux::HIDDevice - Failed to open HIDDevice: %s", hid_path);
        return false;
    }
    ...
    LogText("NervGear::Linux::HIDDevice - Opened '%s'\n"
...
void HIDDevice::HIDShutdown()
{
    ...
    LogText("NervGear::Linux::HIDDevice - HIDShutdown '%s'\n", DevDesc.Path.ToCStr());
}
...
void HIDDevice::closeDevice(bool wasUnplugged)
{
    ...
    LogText("NervGear::Linux::HIDDevice - HID Device Closed '%s'\n", DevDesc.Path.ToCStr());
}

//-----------------------------------------------------------------------------
void HIDDevice::closeDeviceOnIOError()
{
    LogText("NervGear::Linux::HIDDevice - Lost connection to '%s'\n", DevDesc.Path.ToCStr());
    closeDevice(false);
}
                                     ...
                                     bool* error)
{
    ...
    if (messageType == Message_DeviceAdded && DeviceHandle < 0)
    {
        ...
        {
            LogError("NervGear::Linux::HIDDevice - Failed to reopen a device '%s' that was re-added.\n",
                     device_path);
            *error = true;
            return true;
        }

        LogText("NervGear::Linux::HIDDevice - Reopened device '%s'\n", device_path);
