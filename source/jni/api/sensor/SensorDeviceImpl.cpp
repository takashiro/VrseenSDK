...
SensorDeviceImpl::SensorDeviceImpl(SensorDeviceCreateDesc* createDesc)
    : NervGear::HIDDeviceImpl<NervGear::SensorDevice>(createDesc, 0),
...
{
    if (HIDDeviceImpl<NervGear::SensorDevice>:: initialize(parent))
    {
        openDevice();

        LogText("NervGear::SensorDevice initialized.\n");
...
void SensorDeviceImpl::openDevice()
{
	...
	if (pCalibration != NULL)
	{
		...
		{
			LogText("NervGear::SensorDeviceImpl::openDevice - failed to get device uuid.\n");
		}
...
{
    LogText("NervGear::SensorDevice - Lost connection to '%s'\n", getHIDDesc()->Path.toCString());
    NextKeepAliveTickSeconds = 0;
}

void SensorDeviceImpl::shutdown()
{
    HIDDeviceImpl<NervGear::SensorDevice>::shutdown();

    LogText("NervGear::SensorDevice - Closed '%s'\n", getHIDDesc()->Path.toCString());
}
...
{
    bool                 result = 0;
    ThreadCommandQueue * threadQueue = GetManagerImpl()->threadQueue();
...
{
    VLock::Locker lockScope(GetLock());
    *range = CurrentRange;
}
