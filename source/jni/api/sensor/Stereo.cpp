                                      ...
                                      float extraEyeRotationInRadians /*= 0.0f*/ )
{
    ...
    if ( extraEyeRotationInRadians > 0.0f )
    {
        ...
        extraEyeRotationInRadians = VAlgorithm::Min ( DegreeToRad ( 30.0f ), VAlgorithm::Max ( 0.0f, extraEyeRotationInRadians ) );
        ...
        fovPort.UpTan    = VAlgorithm::Max ( fovPort.UpTan   , ( halfLensDiameter + offsetDownwardsInMeters + extraTranslation ) / ( eyeReliefInMeters + extraRelief ) );
        fovPort.DownTan  = VAlgorithm::Max ( fovPort.DownTan , ( halfLensDiameter - offsetDownwardsInMeters + extraTranslation ) / ( eyeReliefInMeters + extraRelief ) );
        fovPort.LeftTan  = VAlgorithm::Max ( fovPort.LeftTan , ( halfLensDiameter + offsetToRightInMeters   + extraTranslation ) / ( eyeReliefInMeters + extraRelief ) );
        fovPort.RightTan = VAlgorithm::Max ( fovPort.RightTan, ( halfLensDiameter - offsetToRightInMeters   + extraTranslation ) / ( eyeReliefInMeters + extraRelief ) );
    }
    ...
    struct FunctionHider
    {
                                          ...
                                          DistortionRenderDesc const &distortion )
        {
            ...
            for ( int step = 0; step < numSteps; step++ )
            {
                ...
                result.LeftTan  = VAlgorithm::Max ( result.LeftTan,  -tanEyeAngle.x );
                result.RightTan = VAlgorithm::Max ( result.RightTan,  tanEyeAngle.x );
                result.UpTan    = VAlgorithm::Max ( result.UpTan,    -tanEyeAngle.y );
                result.DownTan  = VAlgorithm::Max ( result.DownTan,   tanEyeAngle.y );
            }
            return result;
        }
    };
                                         ...
                                         FovPort inputFovPort )
{
    ...
    resultFovPort.LeftTan  = VAlgorithm::Min ( inputFovPort.LeftTan,  phsyicalFovPort.LeftTan );
    resultFovPort.RightTan = VAlgorithm::Min ( inputFovPort.RightTan, phsyicalFovPort.RightTan );
    resultFovPort.UpTan    = VAlgorithm::Min ( inputFovPort.UpTan,    phsyicalFovPort.UpTan );
    resultFovPort.DownTan  = VAlgorithm::Min ( inputFovPort.DownTan,  phsyicalFovPort.DownTan );
