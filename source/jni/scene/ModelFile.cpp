						...
						const ModelGlPrograms & programs, const MaterialParms & materialParms )
{
	...
	if ( models.isObject() )
	{
					...
					if ( surface.isObject() )
					{
						...
						if ( diffuseTextureIndex >= 0 && diffuseTextureIndex < glTextures.length() )
						{
							...
							if ( emissiveTextureIndex >= 0 && emissiveTextureIndex < glTextures.length() )
							{
										...
										reflectionTextureIndex >= 0 && reflectionTextureIndex < glTextures.length() )
								{
									...
									if ( skinned )
									{
										...
										vInfo( materialTypeString << " skinned reflection mapped material");
									}
									else
									{
										...
										vInfo( materialTypeString << " reflection mapped material");
									}
								}
								else
								{
									...
									if ( skinned )
									{
										...
										vInfo( materialTypeString << " skinned light mapped material");
									}
									else
									{
										...
										vInfo( materialTypeString << " light mapped material" );
									}
								}
							}
							else
							{
								...
								if ( skinned )
								{
									...
									vInfo( materialTypeString << " skinned diffuse only material");
								}
								else
								{
									...
									vInfo( materialTypeString << " diffuse only material");
								}
							}
						}
						else if ( attribs.color.length() > 0 )
						{
							...
							if ( skinned )
							{
								...
								vInfo( materialTypeString << " skinned vertex color material");
							}
							else
							{
								...
								vInfo( materialTypeString << " vertex color material");
							}
						}
						else
						{
							...
							if ( skinned )
							{
								...
								model.Def.surfaces[index].materialDef.programObject = programs.ProgSkinnedSingleTexture->program;
								model.Def.surfaces[index].materialDef.uniformMvp = programs.ProgSingleTexture->uniformModelViewProMatrix;
								vInfo( materialTypeString << " skinned default texture material");
							}
							else
							{
								if ( programs.ProgSingleTexture == NULL )
								...
								model.Def.surfaces[index].materialDef.programObject = programs.ProgSingleTexture->program;
								model.Def.surfaces[index].materialDef.uniformMvp = programs.ProgSingleTexture->uniformModelViewProMatrix;
								vInfo( materialTypeString << " default texture material");
							}
						}
		...
		{
			vInfo( "loading ground collision model.." );
								...
								const MaterialParms & materialParms )
{
	...
	for ( int ret = unzGoToFirstFile( zfp ); ret == UNZ_OK; ret = unzGoToNextFile( zfp ) )
	{
		...
		vInfo( "zip level: " << finfo.compression_method << ", file: " << entryName );
