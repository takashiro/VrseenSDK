...
static VOpenGLTexture LoadRGBTextureFromMemory( const uint8_t * texture, const int width, const int height, const bool useSrgbFormat )
{
...
VOpenGLTexture::VOpenGLTexture(VImage* origImage, const VPath& name, void* mipmapData)
    : VTexture(name), m_ColorFormat(ECF_A8R8G8B8), Image(0), MipImage(0),
    ...
    ReadOnlyLock(false), KeepImage(true)
{
    ...
    {
        Image = createImage(m_ColorFormat, ImageSize);
        origImage->copyTo(Image);
    }
    else
    {
        Image = createImage(m_ColorFormat, TextureSize);
        // scale texture
...
VOpenGLTexture::VOpenGLTexture()
    : VTexture(0), m_ColorFormat(ECF_A8R8G8B8), Image(0), MipImage(0),
    TextureName(0), TargetType(0), InternalFormat(GL_RGBA), PixelFormat(GL_BGRA_EXT),
